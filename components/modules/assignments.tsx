...
function AssignmentCard({ assignment }: { assignment: Assignment }) {
  ...
  return (
    <Card className={cn('hover:shadow-sm transition-all', isOverdue && assignment.status === 'PENDING' && 'border-red-500/40')}>
      <CardContent className="p-4">
        <div className="flex items-start justify-between gap-2 mb-2">
          <p className="text-sm font-semibold leading-tight flex-1 text-foreground">{assignment.title}</p>
          <div className="flex items-center gap-1.5 shrink-0">
            <div className={cn('w-2 h-2 rounded-full', pc.dot)} />
            <span className={cn('text-micro font-semibold', pc.cls)}>{pc.label}</span>
          </div>
        </div>
        <p className="text-mini text-muted-foreground mb-3 line-clamp-2">{assignment.description}</p>
        <div className="grid grid-cols-2 gap-y-1.5 mb-3">
          ...
          <div className="flex items-center gap-1.5 text-micro text-muted-foreground col-span-2">
        ...
        <div className="flex items-center justify-between gap-2">
          <StatusBadge domain="assignment" status={assignment.status} size="sm" />
          {assignment.activityName && (
            <span className="text-micro text-muted-foreground bg-muted px-2 py-0.5 rounded-full truncate max-w-[55%]">
              {assignment.activityName}
            </span>
          )}
        </div>
        ...
        {KANBAN_COLS.map(col => (
          <Card key={col.status} className={cn('border-t-2', col.cls)}>
            <CardContent className="p-3 text-center">
              ...
              <p className="text-mini text-muted-foreground">{col.label}</p>
            </CardContent>
          </Card>
        ))}
      </div>
            ...
            return (
              <div key={col.status} className="space-y-2">
                <div className="flex items-center justify-between px-1">
                  ...
                  <Badge variant="secondary" className="text-micro h-5">{colItems.length}</Badge>
                </div>
